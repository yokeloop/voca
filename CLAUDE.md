...
- **Bootstrap installs dependencies step by step** — Step 0 prompts for the storage root (default `~/.voca`), writes the pointer file at `~/.config/voca/root`, and creates the root directory. Later steps install piper (if missing), always present a language picker first (all unique language prefixes from the HF catalog; `config.language` has no default — only after the first bootstrap is it set — so on a fresh install no entry is marked `(current)` and the user must pick explicitly) and then a voice picker filtered to that language (currently active voice on top when it matches the chosen language; picking a new language rewrites `config.language`; picking a voice installs it and rewrites `config.piperModel` to `bin/<name>.onnx`). When the catalog is unreachable, bootstrap falls back to a curated list of universal voices `[en_US-lessac-medium, en_US-amy-medium, ru_RU-irina-medium, de_DE-thorsten-medium]` — languages derived from that list, Python venv in `<VOCA_HOME>/venv/`, portaudio19-dev (checked via `dpkg -s` and installed via `apt-get` if missing), openWakeWord + pyaudio, and the wake word ONNX model. The wake model (`hey_jarvis_v0.1.onnx`) is copied from the installed openwakeword pip package in the venv; if not found there, it falls back to downloading from GitHub. No stop model is downloaded. Confirmation is requested before each installation.
