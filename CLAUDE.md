...
- **Bootstrap installs dependencies step by step** — Step 0 prompts for the storage root (default `~/.voca`), writes the pointer file at `~/.config/voca/root`, and creates the root directory. Later steps install piper (if missing), always present a voice picker filtered by `config.language` (fetches the HF catalog via `listAvailable`; the currently active voice — read from `config.piperModel` — appears at the top of the list marked `(current)`; falls back to a curated list of universal voices `[en_US-lessac-medium, en_US-amy-medium, ru_RU-irina-medium, de_DE-thorsten-medium]` when the catalog request fails or returns no matches; picking a voice installs it and rewrites `config.piperModel` to `bin/<name>.onnx`), Python venv in `<VOCA_HOME>/venv/`, portaudio19-dev (checked via `dpkg -s` and installed via `apt-get` if missing), openWakeWord + pyaudio, and the wake word ONNX model. The wake model (`hey_jarvis_v0.1.onnx`) is copied from the installed openwakeword pip package in the venv; if not found there, it falls back to downloading from GitHub. No stop model is downloaded. Confirmation is requested before each installation.
